...
const UserProfileInfo = ({user,posts,profileId,setShowEdit}) => {
  return (
    <section className="relative py-4 px-6 md:px-8 bg-white">
            ...
            <div className="flex flex-col md:flex-row items-start justify-between">
                <div>
                    <div className="flex items-center gap-3">
                            <h1 className="text-2xl fomt-bold text-gray-900">{user.full_name}</h1>
                            <Verified className="w-6 h-6 text-blue-500"/>
                    </div>
                    <p className="text-gray-600">{user.username ? `@${user.username}`:'Add a username'}</p>
                </div>
                {/* if user is not on others profile that means he is opening his own Profile
                so we will give the edit button */}
                {
                    !profileId && 
            ...
            <p className="text-gray-700 text-sm max-w-md mt-4">{user.bio}</p>
                ...
                <div className="flex items-center gap-6 mt-6 border-t border-gray-200">
                    <div>
                        ...
                        <span className=" text-xs sm:text-sm font-bold text-gray-500 ml-1">Posts</span>
                    </div>
                     
                     <div>
                        <span className="sm:text-xl font-bold text-gray-900">{user.followers.length}</span>
                        <span className=" text-xs sm:text-sm font-bold text-gray-500 ml-1">Followers</span>
                    </div>
                    <div>
                        <span className="sm:text-xl font-bold text-gray-900">{user.following.length}</span>
                        <span className=" text-xs sm:text-sm font-bold text-gray-500 ml-1">Following</span>
                    </div>
                </div>
        </div>
